...
class PengaduanController extends Controller
{
    ...
    public function getAllPengaduan($limit = NULL, $offset = NULL)
    {
        if($this->user->level == 'masyarakat'){
            ...
            if($limit == NULL && $offset == NULL){
                $data["pengaduan"] = Pengaduan::where('id_user', '=', $this->user->id)->orderBy('tgl_pengaduan', 'desc')->
                with('kategori', 'tanggapan','user')->get();
            } else {
                $data["pengaduan"] = Pengaduan::where('id_user', '=', $this->user->id)->orderBy('tgl_pengaduan', 'desc')->
                with('kategori', 'tanggapan','user')->take($limit)->skip($offset)->get();
            }
        } else {
    ...
    public function getByUsers($id_users)
    {   
        $data["pengaduan"] = Pengaduan::where('id_pengaduan', $id_users)->with(['kategori','tanggapan'])->get();
    ...
    public function delete($id_pengaduan)
    {
        ...
        if($delete){
            return $this->response->successResponse('Data pengaduan berhasil dihapus');
        } else {
            return $this->response->errorResponse('Data pengaduan gagal dihapus');
        }
    }
    ...
    public function findPengaduan($limit = NULL, $offset = NULL)
    {
            $data["count"] = Pengaduan::where('id_user', '=', $this->user->id)->count();
